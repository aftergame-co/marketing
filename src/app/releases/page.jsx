...
const changelogs = [
  {
    ...
    blurb:
      "BGG auto-sync, BG Stats import (beta), export all your data, assign your tickets, new premium plans, and so much more in this new update packed full of goodies.",
    features: [
      `[Aftergame Plus] You can now set up your account to automatically pull collection updates and/or play logs from BGG.`,
      `BG Stats import has FINALLY been released in an early beta. During the beta, it is available only to Aftergame Plus subscribers, but we will open this to everyone soon.`,
      `We have now split our premium plan into two: Aftergame Plus (for players) and Aftergame Star (for organizers).`,
      `[Aftergame Star] You can now attach your subscription to a group, giving all group admins access to premium features.`,
      `[Aftergame Star] We have increased the included attendee limit to 200, and for events using Aftergame to sell tickets, there are no longer any limits!`,
      `Export your game collection as a CSV.`,
      `Export your play history as a CSV.`,
      `Event hosts can now export the full attendee list (including all ticket data) as a CSV.`,
      `Event hosts can now export the full schedule as a CSV.`,
      `Event hosts can now export the full game library lending history as a CSV.`,
      `Group admins can now export the full member list as a CSV.`,
      `We have moved buying tickets to its own screen and improved the UX.`,
      `You can now assign any purchased ticket to other players, both during checkout, or after-the-fact.`,
      `[Aftergame Star] You can now embed ticket purchasing directly into your website, allowing customers to buy tickets from your site.`,
      `Group admins can now add multiple named links to other websites and social profiles.`,
      `Group admins can now add a contact email to their group details.`,
      `We now show which games you are in the draw for in Play to Win competitions.`,
      `We now show external links to BGG, RPGG, DriveThruRPG, and soon BGA on the game page.`,
    ],
    fixes: [
      `Greatly improved the speed and reliability of imports.`,
      `Fixed a bug which would attempt a BGG import even if the username did not exist on BGG`,
      `Fixed a bug which would lose the scroll position of the event schedule when clicking into an event.`,
      `Fixed a bug that showed cancelled games as 'Happening now'.`,
      `Fixed a bug which would show a group admin as the host for a game when they had marked themselves as 'Interested'.`,
      `Fixed some bugs with the 'View ticket details' sheet display on the attendees screen.`,
...
...and there's so much more in the full notes.
  ...
  },
  {
    ...
    blurb:
      "Events have had a massive refresh, bringing many new highly requested features. Also added is the ability to connect/disconnect sign in methods and new app preferences.",
    features: [
      `The events and planned game screen has been redesigned, making better use of space and better emphasising important information.`,
      `You can now add and remove co-hosts to events and planned games.`,
      `Planned games can now have a banner image added, perfect for showing off RPG artwork or branding your event game sessions.`,
      `We brought back the ability to request a teacher for your planned game and added the ability to request a Game Master.`,
      `Planned games now support Judges.`,
      `You can now add multiple Teachers, Game Masters, and Judges (and you can also do this while creating the planned game!)`,
      `Teachers, Game Masters, and Judges can now also claim a seat and be a player too.`,
      `When setting up a planned game for any game that is defined, you can now set which ruleset, scenario, campaign, or map will be played.`,
      `Play logs also now support multiple Teachers, Game Masters, and Judges and allows them to also be a player.`,
      `Event hosts can now choose to cancel or fully delete an event. Cancelling an event or game will keep it on the schedule, labelled as Cancelled.`,
      `Event hosts can now give a reason when cancelling or deleting an event.`,
      `When inviting players to a game at an event, we have added a filter to only show event attendees.`,
      `You can now finally connect or disconnect a sign in method, and these do not all need to have the same email address.`,
      `We have added a new 'Preferences' section in settings. This allows you to choose between 12hr/24hr time and imperial/metric units of measurement.`,
    ],
    fixes: [
      `Fixed an issue where a public play log linked to a private event would allow anyone to click on the linked event.`,
      `Fixed issues with mention user search, both where it didn't scroll on web and improved the relevance of results.`,
      `Fixed a UX issue where deleting an event/planned game would keep you on that game and look like nothing happened.`,
      `Fixed an issue where deleting event tickets would lose some ticket purchase details.`,
      `Fixed an issue loading event schedules from a different time zone not bucketing the days correctly.`,
      `Fixed some issues with setting the date/time of events on web.`,
      `Fixed an issue with longer ticket descriptions being cut off.`,
      `Fixed an issue which blocked the ability to edit $0 tickets.`,
      `Fixed a bug where an event without a location posted in a group would not notify group members.`,
      `Various other fixes and performance improvements.`,
    ],
  },
  {
    ...
    features: [],
    fixes: [
      `Ticketing beta: Fixed an issue where only 1 seat was able to be claimed even if you bought multiple tickets to the main event.`,
      `Improved event clash detection buffer check.`,
      `Various other fixes and performance improvements.`,
    ],
  },
  {
    ...
    fixes: [
      `Ticketing beta: fixed an error which displayed "no such payment_intent".`,
      `Ticketing beta: ensure attendees purchase a main ticket when buying an add-on.`,
      `Ticketing beta: fixed a bug which blocked signing up for games within a ticketed event in some cases.`,
      `Ticketing beta: improved the display of tickets with longer descriptions.`,
      `Ticketing beta: editing tickets would ask if you wanted to discard changes after saving.`,
      `Fixed a bug where deleting a game list would not remove it until app refresh.`,
      `Fixed a bug where the bottom game filter was cut off on the web app.`,
      `Various other fixes and performance improvements.`,
    ],
  },
  {
